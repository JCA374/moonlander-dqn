...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description='FIXED LunarLander-v3 Training')
    ...
    parser.add_argument('--max-steps', type=int, default=600)  # FIXED: Reasonable limit
    parser.add_argument('--batch-size', type=int, default=64)
    parser.add_argument('--memory-size', type=int, default=50000)
    parser.add_argument('--gamma', type=float, default=0.99)  # FIXED: Proper gamma
    parser.add_argument('--epsilon', type=float, default=1.0)
    parser.add_argument('--epsilon-min', type=float, default=0.01)
    parser.add_argument('--epsilon-decay', type=float, default=0.995)  # FIXED: Better decay
...
def optimized_train_loop(env, agent, args, logger):
    ...
    for episode in range(args.episodes):
        ...
        for step in range(args.max_steps):
            ...
            if reward > 100:  # FIXED: Successful landing (was looking for >200)
            ...
            if done:
                ...
                if terminated and original_reward > 0:  # FIXED: Was checking >200
                    ...
                    logger.info(f"Episode {episode}: SUCCESSFUL LANDING! Original reward: {original_reward:.2f}, Scaled: {scaled_reward:.2f}")
    ...
    logger.info(f'Successful Landings: {successful_landings}/{args.episodes} ({success_rate:.2%})')
    ...
    if not args.restart:
        ...
        if model_path:
            try:
                ...
                eval_reward, _, success_rate = evaluate(env, agent, 5, args.seed, args.max_steps)
